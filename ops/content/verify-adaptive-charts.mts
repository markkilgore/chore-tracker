...
import { addDays, weeklySpeciesLesson, weekDates, chartCheckboxBox } from "../../packages/domain/src/index";
  ...
  for (const theme of ["cats", "shark"]) for (let week = 0; week < 24; week++) {
    for (const count of week === 0 ? [0, 1, 2, 4, 5, 8, 9, 12, 13, 24, 25] : [2, 12]) {
      ...
      const snapshot: ChartSnapshot = { chartId: "example", householdName: "Our family", memberId: theme,
        memberName: theme === "cats" ? "Kate" : "Henry", weekStartDate: date, themeKey: theme, planRevision: 1, layoutVersion: 3,
      ...
      const pages = await page.locator(".chart-page").evaluateAll(pages => pages.map(section => {
      ...
      for (const [index, sheet] of pages.entries()) {
        ...
        sheet.boxes.forEach((box, cell) => {
          const expected = chartCheckboxBox(Math.min(12, count - index * 12), Math.floor(cell / 7), cell % 7);
